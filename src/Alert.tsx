...
const Alert = ({ type, message, onOk, onCancel }: any) => {
    ...
    return isVisible ? (
      <>
        {type === 'question' && <div className="fixed inset-0 bg-opacity-50"></div>}
          ...
          <div className={`${bgColor} rounded-lg p-4`}>
            ...
            {button &&
              <div className="flex justify-end">
                <button className="mr-2 bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded transition-all" onClick={handleOk}>OK</button>
                <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded transition-all" onClick={handleCancel}>Cancel</button>
              </div>
            }
          </div>
        </div>
